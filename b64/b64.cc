...
auto enc_nonurlsafe(std::vector<uint8_t>& inbuf) -> std::string {
	size_t out_len = 0;
...
auto dec_nonurlsafe(std::string_view s) -> std::vector<uint8_t> {
	size_t max_len = 0;
...
auto dec(std::string const& s) -> std::vector<uint8_t> { return dec(std::string_view{s}); }
