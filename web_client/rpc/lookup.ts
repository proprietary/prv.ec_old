...
const { LookupResponse } = lookup_response.ec_prv.fbs;
import { post } from './common';

export async function lookupRequestV1(
	urlIndex: lookup_request.ec_prv.fbs.LookupRequestT,
): Promise<null | lookup_response.ec_prv.fbs.LookupResponseT> {
	throw new Error('unimplemented');
}

export async function lookupRequestWeb(
	urlIndex: string,
): Promise<null | lookup_response.ec_prv.fbs.LookupResponseT> {
	...
	const lookupResponse = LookupResponse.getRootAsLookupResponse(
